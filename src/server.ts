...
// ---- 三端 App 的更新接口 + 从本站发安装包 ----
// 这四条路由抽到了 src/web/app_downloads.ts。抽出去的理由写在那个文件头上,
// 一句话:写在这里的话**集成测试注册不了它们**,于是它们一次都没被真正打过——
// 而「传到本地的包对 App 自动更新毫无作用」那条 bug 就是这么活下来的。
{
  const { appDownloadRoutes } = await import("./web/app_downloads.js");
  await app.register(appDownloadRoutes);
}
